...
printWelcome();
  let exit = false;
  while (!exit) {
      const choice = await meShell.selectMe();
      switch (choice) {
         /**
         ...
         case 'View Existing Profiles':
          const selectedProfile = await meShell.selectProfile();
          if (selectedProfile === 'back') {
              break;
          }
          // Read and display the selected profile details here
          console.log(`Selected profile: Name: ${selectedProfile.name}, Lastname: ${selectedProfile.lastname}, Birthday: ${selectedProfile.birthday}`);
          // Add logic to handle the selected profile
          break;
          case 'Create New Profile':
            const newProfile = await meShell.createMe(); // Or createMe if that's the name you're using
